...
object Formatter {
  ...
  /** Accepts a list of (argument line, option description, optional default value). 
    * Also accepts optional width, to which the result must be formatted.
    */
  def format(s: List[(String, String, Option[String])], width: Option[Int] = None): String = {
    ...
    s.flatMap { case (arg, descr, defVal) => 
      val argPadding = " " * (if (arg.trim.startsWith("--")) 4 else 0)
      val text = wrap(descr.split(" ") ++ defVal.map("(default = %s)" format _), neededWidth - argWidth - COLUMN_PADDING).map(l => " " * (argWidth + COLUMN_PADDING + INDENT) + l)
